"""
seed_s3.py (Joe 0630) — seed the s3 family at TF180 for the KERNEL test (does s3r prediction sharpen entry MAE).
s3m = s6m cloned with bb_mult 0.56 · s3M = s6M (37|0.72|ohlc4) · s3r = s6r (k 5|6|6|close). All CLOSED-bar. Idempotent.
"""
...
if ic('s3r') is None:
    itf = db.execute("SELECT itf_pk FROM indicator_timeframes WHERE itf_seconds=180", fetch=True)
    itf3 = itf[0]['itf_pk'] if itf else None
    if itf3 is None:
        db.execute("INSERT INTO indicator_timeframes (itf_label, itf_seconds) VALUES ('3', 180)")
        itf3 = db.execute("SELECT itf_pk FROM indicator_timeframes WHERE itf_seconds=180", fetch=True)[0]['itf_pk']
    ...
    for src, mult_override in [('s6m', 0.56), ('s6M', None), ('s6r', None)]:
        ...
        db.execute('''INSERT INTO indicator_configs (ic_is_pk,ic_il_pk,ic_itf_pk,ic_line_type,ic_live_after_dt,ic_src,
            ...
            (s['ic_is_pk'], s['ic_il_pk'], itf3, s['ic_line_type'], s['ic_src'], s['ic_high_boundary'], s['ic_low_boundary'],
             s['ic_bb_len'], bb_mult, s['ic_k_len'], s['ic_rsi_len'], s['ic_stc_len'], closed, s['ic_wobble']))
print('s3 family:', db.execute(
    "SELECT ind_name, value_mode, itf_seconds FROM pk_optimizer.vw_indicator_configs_live WHERE ind_name LIKE 's3%' ORDER BY ind_name", fetch=True))
db.disconnect()
