"""build_wsf_pxs_momo_flip_rep — the table behind the wsf-momo-flip-rep report.

WHY. Joe 0904: "can you build a wsf_pxs_momo_flip_rep table that's created by all lines using
src=pxs instead of src=close?"

COLUMN NAMES MATCH wsf_momo_flip_rep EXACTLY. Joe 0905: "you've made changes to the column
names. please keep them the same as wsf_momo_flip_rep". The two tables differ only in the rows
they hold, so a query written against one runs against the other unchanged.

THE SAME REPORT, A DIFFERENT PRICE UNDER IT. Every row here is produced by the SAME rules, knobs
and layout as wsf_momo_flip_rep - flip_report.py --pxs runs one file, not a copy. What differs is
the line source: build_pxs_line_cache.py rebuilt all 160 wsf lines on a base whose `close` column
is the smoothed price, in .rpl_cache/lines_pxs. Joe 0904 chose that route: "agreed on option (b)".

THE SPAN IS THE CACHE'S, NOT A HORIZON I PICKED. Joe 0904: "we only need 08-04 to 08-06 for now,
plus the warmup". The pxs cache ends 2026-08-05 23:59:55, so a forward search that would have run
past it in the close table reads `none` here. Those cells are NOT comparable against
wsf_momo_flip_rep, which searches to 08-19.
...
then, not now.
    ...
    python3 build_wsf_pxs_momo_flip_rep.py            create the table if absent
    python3 build_wsf_pxs_momo_flip_rep.py --show     print what is banked, newest knob set first
"""
import sys
...
DDL = '''CREATE TABLE IF NOT EXISTS wsf_pxs_momo_flip_rep (
...
UPSERT = (f"INSERT INTO wsf_pxs_momo_flip_rep ({', '.join(COLS)}) "
...
def KNOBS(variant, m_wob, xwob_cross, xwob, oob_hi, oob_lo, bank, mt, pxs):
    ...
    `pxs` is the dema length the smoothed price was built at - the term that says these rows are
    NOT close-sourced. `mt` is the momentum-true test the ROW SET was built on. 'cm' is dtf curl or dtf momo, Joe
    ...
    return (f"v{variant}_mw{int(m_wob)}_xwc{int(xwob_cross)}_xw{int(xwob)}"
            ...
            f"_r2{_n(bank['momo_r2_min'])}_mt{mt}_px{pxs}")


def create(db):
    db.execute(DDL)
    print('wsf_pxs_momo_flip_rep ready')


def show(db):
    rs = db.execute(
        "SELECT wmf_knobs k, COUNT(*) n, SUM(wmf_dirty) d, MIN(wmf_utc) a, MAX(wmf_utc) b, "
        "MAX(wmf_built_at) t FROM wsf_pxs_momo_flip_rep GROUP BY 1 ORDER BY t DESC", fetch=True)
    if not rs:
        print('wsf_pxs_momo_flip_rep is empty')
        return
