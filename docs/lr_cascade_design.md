...
- **s30M** — *was* the trade trigger (closed-bar 2-wob); **DROPPED 0703** — data-confirmed **subsumed** by the s30a latch (a separate wobslay drops ~half the qualified gate-opens for a net-negative: worst-window −28→−34, mean +11→+9). Now just a *component* of s30a. **Replacement planned: `gcs5`** — a 5s clone of s15 (m 10·0.6·hlc3 · M 37·0.83·ohlc4 · r 5·6·6·hl2), fired as `gcs5a` AFTER s30a+s15a and triggering on **gcs5Mage reversal (wob 1)**. 5s-native ⇒ every bar is a real slope step (no coarse-bar flats), so wob1 IS the turn; and at base-TF there's no emerging/closed gap ⇒ **inherently look-ahead-free**.
...
4. Gate open → **FINISHER DELATCH**: the finishers **latch s30a AND s15a breaches from the ARM onward** (s15a = ic 84/85/86, the TF15 twin of s30a; each honouring its own **r-lookback** — how far back it scans for a same-side r breach, as **two independent DB knobs**: `s30r_lookback`=`lp_s30r_lb`=**19**, `s15r_lookback`=`lp_s15r_lb`=**29** (split from the old single hardcoded `rlb=19`, Joe 0703 — recovers late-s15r setups like 06-17 05:44/07:18)) and breach at their OWN times — s15a can lead s30a by minutes, the latch carries each. Trade fires once **both have latched**, never before the gate-open → **trade_k = max(latched, gate-open)**: both pre-latched ⇒ trade AT the gate-open ("trade immediately"); a late finisher ⇒ trade when it latches (forward). **No drop — every gate-open trades once both latch.**
5. **s30M is a *component* of the s30a latch** — no separate wobslay trigger; the delatch (gate-open) IS the entry.
   - **Finisher window (spec intent, Joe 0703):** at the s2Mage reversal (gate-open) the finisher walks **BACK 4×30s bars** for s30a+s15a (each honouring its own r-lookback) — **trade immediately** if both signalled inside that window — then walks **FORWARD with 2×30s tolerance** so a line that's *late* still contributes. The **4×30s-back / 2×30s-fwd window is a distinct mechanic from the r-lookbacks** (it's the walk-around-the-reversal, not the same-side-r-breach scan). The current **latch model** approximates this (latches s30a+s15a from the ARM onward = a wider back-window; forward-until-both-latched via `trade_k = max(latched, gate-open)`) — **reconcile the exact 4×30s/2×30s window separately** (open item).
