...
- ✓ **[4] finisher** (CORRECTED 0630) — NOT a reject filter (my first build was a 3-min s30a-AND-s15a-AND-wob drop that killed ~79% — Joe: not in spec, I inferred it; it accidentally quality-selected). Correct: **WAIT for the gate = s30a AND s15a both active** (es-side, r-lookbacks; lookback 4×30s + fwd to the setup cap), then the trade = **the first s30M-wob toward bd at-or-after the gate** — s30M is a *component* of s30a, so within seconds of the gate it turns into the wob. **No drop — every gate-open (every s5m breach) trades.** s30M-wob = slope-flip held 2 bars (the deb2 mechanic; std wobslay is dead on the closed s30M step-line). `gate_open` threads the setup cap.
...
**v2 EXIT (`lr_v2_exit_ab.py`):** v2 entries → `lr_exit` (s7·`exit_on=s30a_s15a`·predict_gate=on·s5m=0.4, NO bias). With the CORRECTED finisher (every breach trades): **252 trades, medMAE 0.36%, 69% win, avg +0.164%/trade, +41.4% gross net** (exits: finisher/SL/end). **HONEST: NET-NEGATIVE after ~0.20% RT cost (≈ −0.04%/trade).** The earlier "+0.259%/75%/+44% / first costed edge" was the **buggy drop-filter flattering it** — it kept the cleaner 131 subset (medMAE 0.30/75%); the full correct set is 0.36/69%. **As designed the v2 trades too much for a + net-of-cost — ENTRY SELECTION (the bias) is the lever, not upside.** AB *verdict* (0.4>0.65 · `s30a_s15a`≫`curl` · `predict_gate=on`) likely holds (config-relative); re-run the exit AB on the corrected 252 entries to confirm absolutes. Table = `v2_walk` (cf15 cols + exit_pct/reason); pine = `v2_trades.pine`.
