...
**v2 EXIT (#45 resolved, `lr_v2_exit_ab.py`):** the v2 entries → `lr_exit` (s7) are **NET POSITIVE under the SL** — the first costed edge end-to-end. Best *reliable* config = **s7/s8 · `exit_on=s30a_s15a` · predict_gate=on · s5m=0.4**. DEDUPED (v2_walk double-counted ~16% — two arm setups → one trade; fixed in v2_walk): **131 trades, avg +0.259%/trade, 75% win, +33.9% net** (exits: 99 finisher / 31 SL / 1 end). Findings (verdict robust to the dedup): every config nets + · `predict_gate=on` wins (the ride) · `s30a_s15a` ≫ `curl` (same avg, **75% vs ~50% win**) · curl_fam minor. **Caveat: GROSS** — at the ~0.20% RT cost net-of-cost ≈ **+0.06%/trade** (thin but +). The 75% win + medMAE-0.30 entries are the foundation; **no hb33/upstream bias applied yet** (the unbuilt lever). Table = `v2_walk` (cf15 cols + exit_pct/reason); pine = `v2_trades.pine`.
