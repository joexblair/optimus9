...
**1b. The strategy (SRP: decide) — collector-triggered.** When the collector (1a) prints a 5s kline (the kline build IS the clock — no separate timer/loop) it triggers → `run_window(now − BUFFER, now)` over a **bounded rolling buffer** — BUFFER = longest line lookback + cascade horizon (≈ hours, NOT the backtest's 7-day span; `BiasWindow` is a *backtest batch* construct, never a live recompute) → reads the latest-bar decision → an **intent** (`open/add/close`, 66k). Same validated code on a tiny window ⇒ **live == backtest by construction**, recompute trivial. Never touches the exchange. *(Derive BUFFER by tracing the longest-lookback line — the one measurement that matters.)*
...
4. o9-live loop (bounded-buffer `run_window` → cf15 → intent → adapter).
