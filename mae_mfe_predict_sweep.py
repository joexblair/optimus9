...
INTERPRETATION (Joe: correct any before trusting it) — anchor per m-line OOB breach ("the arm"):
  ...
  * PREDICTED    -> anchor = first bar AFTER the predict where r slope-flips toward the trade side (bd=-es),
                   BOUNDARY-AGNOSTIC (no OOB requirement — the s2Mage principle; engine's _slope_flip). Joe 0707.
...
WOB = 6                          # r-reversal confirmation (Joe: <6 = premature reverses; 6 = healthy start)
...
def main():
    ...
    for p, tf, mmult, Mmult in TFS:
        ...
        rrev = np.asarray(_mage_rev(r, WOB))                         # engine r-reversal; boundary-agnostic, wob-confirmed
        for L in LENS:
            ...
            for b, es in arms:
                ...
                if predicted:
                    pf = next((k for k in range(b, e) if pred[k] == es), b)     # first predict bar in the episode
                    rev = next((k for k in range(pf, e) if rrev[k] == -es), None)   # first r-reversal toward bd (=-es)
                ...
                rows.append((p, L, WOB, int(ts[b]), _dt(ts[b]), es, int(predicted), atype,
    ...
    dev.execute("""CREATE TABLE mae_mfe_sweep (
        id BIGINT AUTO_INCREMENT PRIMARY KEY, tf VARCHAR(4), m_len TINYINT, wob TINYINT, arm_ms BIGINT, arm_dt DATETIME,
    ...
    dev.executemany("INSERT INTO mae_mfe_sweep (tf,m_len,wob,arm_ms,arm_dt,es,predicted,anchor_type,anchor_ms,mae,mfe,"
                    "swing_ms) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", rows)
