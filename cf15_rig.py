"""
cf15_rig.py (Joe 0627) — the cf15 latch-release walk engine. run_window() is the parameterized window
walk (reused by cf15_superscope.py for the 8-window baseline); main() runs the single 06-17→06-22 window
and writes cf15_walk.
...
def run_window(db, start_ms, end_ms, floor=FLOOR, wob_n=WOB_N):
    """Detect 15:24 setups up to end_ms with trades gated to >= start_ms, walk each. Returns rows of
    (trade_ms, dt, es, bd, mae, mfe, mfe_ok, mfe_swing_side, price). Parameterized window — the superscope
    loops this; main() calls it for the single 06-17→06-22 window."""
    ...
    W = bm.BiasWindow(db, end_ms, cfg=cfg); ts = W.ts; n = len(ts)
        ...
        if sign[i] != 0 and sign[i] != sign[i - 1]:          # s6m OOB onset (closed), side es
            ...
            if rj is not None:
                ...
                if tj is not None and int(ts[tj]) >= start_ms:
                    trades.append((int(ts[tj]), es, -es, int(tj)))
            i = next((k for k in range(i + 1, n) if sign[k] != es), n)
            continue
        i += 1

    rows = []
    for tms, es, bd, tj in trades:
        ...
        rows.append((tms, dts(tms), es, bd, round(mae, 3), round(mfe, 3), int(mfe >= TARGET), mfe_side, float(W.px[tj])))
    return rows
